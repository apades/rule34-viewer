...
type RuleJsonListValues<T = dykey> = RuleListValues & {
  $: T[]
}
type RuleXmlListValues = RuleListValues & {
  $q: $q
}

// --##item
type RuleItemValues = RuleListValues & {
  index: number
}
type RuleItemInListValues<T = dykey> = RuleItemValues & {
  $i: T
  $list: T[]
}
